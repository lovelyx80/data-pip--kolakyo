...
val baseApplicationId = "com.lovelybeats.music"
...
android {
    ...
    productFlavors {
        // FOSS - no gcast, updater disabled (custom update.json system used instead)
        create("foss") {
            dimension = "variant"
            isDefault = true
        ...
        // GMS - gcast, updater disabled (custom update.json system used instead)
        create("gms") {
            dimension = "variant"
...
configurations.configureEach {
    ...
    // Some transitive dependencies (e.g. old, no-longer-maintained AARs such as
    // TAndroidLame, which predates the AndroidX migration) still pull in legacy
    // com.android.support artifacts (support-v4, appcompat-v7, etc). These ship
    // both classes (android.support.v4.app.*, android.support.v4.media.*, ...)
    // and resources (e.g. attr/actionBarSize) that duplicate what androidx.core,
    // androidx.media, and androidx.appcompat already provide — every module in
    // this project already depends on the androidx equivalents. Excluding the
    // whole legacy group avoids both "Duplicate class" failures from
    // checkFossReleaseDuplicateClasses and "Duplicate value for resource"
    // failures from mergeFossReleaseResources, without whack-a-moling individual
    // support-library modules as they surface.
    exclude(group = "com.android.support")
}

dependencies {
    // MP3 encoder (LAME) used to export downloaded songs as real, standalone .mp3 files
